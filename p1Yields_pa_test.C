...
void peakFitter(TFile *TFitOut, const char *fileName, const char *detector,
	...
	TLatex tt(1830,1000000,"E_{p} = 2.4879 MeV");
	tt.SetTextSize(0.05);
	tt.Draw();

	TLatex t(1120,1250,"#bf{#alpha}_{1} \t1.369 MeV");
	t.SetTextSize(0.035);
	TArrow ar(1130,1100,1130,150,0.01,"|>");
	ar.Draw();
	t.Draw("SAME");

	TLatex t1(635,12000,"p_{1} \t0.844 MeV");
	t1.SetTextSize(0.035);
	TArrow ar1(702,10000,702,1000,0.01,"|>");
	ar1.Draw();
	t1.Draw("SAME");

	TLatex t2(775,4000,"p_{2} \t1.015 MeV");
	t2.SetTextSize(0.035);
	TArrow ar2(843,3000,843,1000,0.01,"|>");
	ar2.Draw();
	t2.Draw("SAME");

	hyield->SetLineColor(kBlack);
	TGaxis *axis1 = new TGaxis(2500,1,2500,7300000,1,7300000,10,"+GLB"); //GLB
  	axis1->SetName("axis3");
	axis1->SetLabelSize(0);
  	axis1->Draw();

	TGaxis *axis2 = new TGaxis(0,7300000,2500,7300000,0,2500,25,"-LB"); //GLB
  	axis2->SetName("axis2");
	...
	try {
		...
		// c0->SaveAs(Form("Yields/P1/run_%s/%s_Fit.png",runNum.c_str(),detNum.c_str()));
		// c0->SaveAs(Form("Yields/P1/det-%i/run_%s_Fit.png",detLoop,runNum.c_str()));
		c0->SaveAs(Form("Yields/P1/run_%s/%s_Fit.pdf",runNum.c_str(),detNum.c_str()));
		c0->SaveAs(Form("Yields/P1/det-%i/run_%s_Fit.pdf",detLoop,runNum.c_str()));
...
void p1Yields_pa_test(){
	...
	// 		gSystem->Exec(Form("mkdir TFitResult/P1/det-%i",ii));
	// 	}catch(...){}
	// }
	...
	for(int i=runStart;i<upToRun;i++){
		...
		// try {
		// 	gSystem->Exec(Form("mkdir Yields/P1/run_%s",runNum_String));
		// }catch(...){}

		// Save TFitResult results.
		TFile *ff = new TFile(Form("TFitResult/P1/run_%s.root",runNum_String),"RECREATE");
