...
void peakFitter(const char *fileName, const char *detector,
	...
	try {
		...
		TFitResult fitResults = static_cast<TFitResult>(*triple_gauss_area(hyield).Get());
		// fitResults.Print();
		...
		c0->SaveAs(Form("Yields/A1/run_%s/%s_Fit.png",runNum.c_str(),detNum.c_str()));
		c0->SaveAs(Form("Yields/A1/det-%i/run_%s_Fit.png",detLoop,runNum.c_str()));
...
void a1Yields_pa(){
	...
	// 		gSystem->Exec(Form("mkdir TFitResult/A1/det-%i",ii));
	// 	}catch(...){}
	// }
